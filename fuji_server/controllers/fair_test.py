...
class FAIRTest:
    ...
    def check_searchable(self):
        ...
        search_mechanisms = []
        sources_registry = [ujimap.Sources.SCHEMAORG_NEGOTIATE.value, ujimap.Sources.DATACITE_JSON.value]
        # print('self.metadata_sources ', self.metadata_sources)
        search_engines_support = [ujimap.Sources.SCHEMAORG_EMBED.value, ujimap.Sources.DUBLINCORE.value]
    ...
    def check_license(self):
        ...
        #We assume exactly one licence is passed as string..
        if self.metadata.get('license') is not None and isinstance(self.metadata['license'], str):
