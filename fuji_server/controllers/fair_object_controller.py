...
async def assess_by_id(body):
    ...
    if connexion.request.content_type == "application/json":
        ...
        if ft.repeat_pid_check:
            ...
            ft.set_harvested_metadata()
            ft.clean_metadata()
        await ft.harvest_re3_data()
        ft.harvest_github()
        core_metadata_result = await ft.check_minimal_metatadata()
        # print(ft.metadata_unmerged)
        # print('F-UJI checks: access level')
        access_level_result = await ft.check_data_access_level()
        # print('F-UJI checks: license')
        license_result = await ft.check_license()
        license_file_result = await ft.check_license_file()
        # print('F-UJI checks: related')
        related_resources_result = await ft.check_relatedresources()
        # print('F-UJI checks: searchable')
        check_searchable_result = await ft.check_searchable()
        # print('F-UJI checks: data content')
        ft.harvest_all_data()
        uid_result, pid_result = await ft.check_unique_persistent_metadata_identifier()
        # uid_data_result = ft.check_unique_content_identifier()
        # pid_data_result = ft.check_persistent_data_identifier()
        content_identifier_included_result = await ft.check_data_identifier_included_in_metadata()
        retrievable_result = await ft.check_metadata_data_retrievable()
        data_identifier_included_result = await ft.check_data_content_metadata()

        metadata_identifier_included_result = await ft.check_metadata_identifier_included_in_metadata()
        data_file_format_result = await ft.check_data_file_format()
        upid_software_result = await ft.check_unique_persistent_software_identifier()
        software_component_result = await ft.check_software_component_identifier()
        version_identifier_result = await ft.check_version_identifier()
        development_metadata_result = await ft.check_development_metadata()
        open_api_result = await ft.check_open_api()
        requirements_result = await ft.check_requirements()
        test_cases_result = await ft.check_test_cases()
        # print('F-UJI checks: data file format')
        community_standards_result = await ft.check_community_metadatastandards()
        data_provenance_result = await ft.check_data_provenance()
        code_provenance_result = await ft.check_code_provenance()
        formal_metadata_result = await ft.check_formal_metadata()
        # print('F-UJI checks: semantic vocab')
        semantic_vocab_result = await ft.check_semantic_vocabulary()
        metadata_preserved_result = await ft.check_metadata_preservation()
        standard_protocol_data_result = await ft.check_standardised_protocol_data()
        standard_protocol_metadata_result = await ft.check_standardised_protocol_metadata()
        standard_protocol_auth_result = await ft.check_standardised_protocol_authentication()
        if uid_result:
