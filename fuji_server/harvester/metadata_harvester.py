...
from fuji_server.helper.signposting_helper import SignpostingHelper
...
class MetadataHarvester:
    ...
    signposting_relation_types = SignpostingHelper.signposting_relation_types
    ...
    def __init__(
        ...
        ##### helper
        self.ch = CompatibiltyHelper(metric_version=metric_version)  # TODO: move to metric helper
        self.signposting_helper = SignpostingHelper(metric_version=metric_version)
    ...
    async def merge_metadata(self, metadict, url, method, format, mimetype, schema="", namespaces=[]):
        try:
            ...
            metadata_standard = self.metadata_helper.get_metadata_standard_by_uris(test_uris)
            allow_merge = True
                ...
                if metadict.get("object_identifier"):
                    ...
                    for object_identifier in metadict.get("object_identifier"):
                        ...
                        if (
                            ...
                            pid_record = await pid_helper.get_identifier_info(self.pid_collector)
    ...
    def get_signposting_object_identifier(self):
        ...
        signposting_pid_link_list = self.signposting_helper.get_links("cite-as")
            ...
            try:
                self.logger.info(
                    "{} : Trying to identify EMBEDDED  Microdata, OpenGraph or Schema.org -: {}".format(
                        self.ch.get_metric("metadata_properties"), self.landing_url
                    )
                )
    ...
    async def download_and_init_content(self):
        response_status = None
        try:
            self.logger.info(
                self.ch.get_metric("metadata_properties") + " : Trying to resolve input URL -: " + str(self.id)
            )
        ...
        if self.landing_url:
            if self.landing_url not in ["https://datacite.org/invalid.html"]:
                if response_status == 200:
                    ...
                    if self.is_html_page:
                        ...
                        if (
                            ...
                            rendered_html = await requestHelper.render_page(
                                metric_id=self.ch.get_metric("metadata_properties")
                            )
                            if not rendered_html:
                                self.logger.warning(
                                    self.ch.get_metric("metadata_properties")
                                    + " : Headless browser returned no rendered HTML"
                                )
                            else:
                                render_ratio = round(100 - (len(self.landing_html) / len(rendered_html) * 100), 2)
                                if render_ratio > 10:
                                    self.logger.warning(
                                        self.ch.get_metric("metadata_properties")
                                        + " : The HTML code generated by JS accounts for more than -: "
                                        + str(render_ratio)
                                        + "% "
                                    )
                ...
                self.logger.warning(
                    self.ch.get_metric("pid") + " : Invalid DOI, identifier resolved to -: " + str(self.landing_url)
                )
                self.landing_url = None
        try:
    ...
    async def retrieve_metadata_embedded(self):
        ...
        await self.download_and_init_content()

        if self.landing_url and self.is_html_page:
            # set typed and signposting links if content could be identified
            if (
                ...
                if "html" in str(self.landing_content_type):
                    ...
                    source_schemaorg, schemaorg_dict = await schemaorg_collector_embedded.parse_metadata()
                    ...
                    await self.extract_embedded_dublin_core(self.landing_html)

                    # ========= retrieve embedded rdfa and microdata metadata ========

                    await self.extract_embedded_rdfa(self.landing_html)
                    await self.extract_embedded_microdata(extruct_metadata)

                    # ========= retrieve highwire and eprints embedded in html page =========

                    await self.extract_embedded_highwire(self.landing_html)
                    ...
                    source_opengraph, opengraph_dict = await opengraph_collector.parse_metadata()
                    ...
                    if opengraph_dict:
                        ...
                        self.logger.info(
                            self.ch.get_metric("metadata_properties") + " : OpenGraph metadata UNAVAILABLE"
                        )
    ...
    async def set_typed_links(self):
        if self.landing_url and self.is_html_page:
            await self.signposting_helper.set_all_typed_and_signposting_links(
                self.landing_url, self.pid_url, self.landing_html, self.landing_headers
            )
            ...
            if self.is_harvesting_method_allowed(
                ...
                data_sign_links = self.signposting_helper.get_links("item", origin="header")
                if data_sign_links:
                    self.logger.info(
                ...
                self.logger.warning(
                    self.ch.get_metric("metadata_properties") + " :Skipping typed or signposting link collection"
                )
    ...
    async def retrieve_metadata_external_rdf_negotiated(self, target_url_list=[]):
        ...
        if self.is_harvesting_method_allowed(MetadataOfferingMethods.CONTENT_NEGOTIATION):
            ...
            for targeturl in target_url_list:
                ...
                if neg_rdf_collector is not None:
                    source_rdf, rdf_dict = await neg_rdf_collector.parse_metadata()
                    ...
                    if rdf_dict:
                        ...
                        self.logger.info(
                            self.ch.get_metric("metadata_properties") + " : Linked Data metadata UNAVAILABLE"
                        )
        else:
            self.logger.info(
                self.ch.get_metric("metadata_properties")
    ...
    async def retrieve_metadata_external_schemaorg_negotiated(self, target_url_list=[]):
        if self.is_harvesting_method_allowed(MetadataOfferingMethods.CONTENT_NEGOTIATION):
            for target_url in target_url_list:
                ...
                source_schemaorg, schemaorg_dict = await schemaorg_collector_negotiated.parse_metadata()
                schemaorg_dict = self.exclude_null(schemaorg_dict)
                if schemaorg_dict:
    ...
    async def retrieve_metadata_external_xml_negotiated(self, target_url_list=[]):
        if self.is_harvesting_method_allowed(MetadataOfferingMethods.CONTENT_NEGOTIATION):
            # print('TARGET URLS:',target_url_list)
            for target_url in target_url_list:
                self.logger.info(
                ...
                source_neg_xml, metadata_neg_dict = await negotiated_xml_collector.parse_metadata()
    ...
    async def retrieve_metadata_external_oai_ore(self):
    ...
    async def retrieve_metadata_external_datacite(self):
        if self.is_harvesting_method_allowed(MetadataOfferingMethods.CONTENT_NEGOTIATION):
            # if self.pid_scheme:
            ...
            if datacite_target_url:
                ...
                source_dcitejsn, dcitejsn_dict = await dcite_collector.parse_metadata()
                ...
                if dcitejsn_dict:
                    ...
                    self.logger.info(self.ch.get_metric("metadata_properties") + " : Datacite metadata UNAVAILABLE")
            else:
                self.logger.info(
                    self.ch.get_metric("metadata_properties")
    ...
    def get_connected_metadata_links(self):
        ...
        if self.is_harvesting_method_allowed(MetadataOfferingMethods.SIGNPOSTING) or self.is_harvesting_method_allowed(
            ...
            signposting_html_links = self.signposting_helper.get_links(
                "describedby", "content"
            )  # self.get_html_typed_links(["describedby"])
            # signposting header links
            if self.is_harvesting_method_allowed(MetadataOfferingMethods.SIGNPOSTING):
                signposting_header_links = self.signposting_helper.get_links(
                    "describedby", ["header", "linkset"], False
                )
            ...
            if self.is_harvesting_method_allowed(MetadataOfferingMethods.TYPED_LINKS):
                html_typed_links = self.signposting_helper.get_links(
                    ["meta", "alternate meta", "metadata", "alternate"], "content", False
                )
    ...
    async def retrieve_metadata_external_linked_metadata(self):
        ...
        if typed_metadata_links:
            ...
            typed_metadata_links = self.sort_link_list(typed_metadata_links)
                ...
                if re.search(
                    ...
                    if self.is_harvesting_method_allowed(
                        ...
                        if typed_rdf_collector is not None:
                            source_rdf, rdf_dict = await typed_rdf_collector.parse_metadata()
                    ...
                    if self.is_harvesting_method_allowed(
                        ...
                        if linked_xml_collector is not None:
                            source_linked_xml, linked_xml_dict = await linked_xml_collector.parse_metadata()
        ...
        if (
            ...
            if self.landing_url or self.pid_url:
                ...
                if target_url_list:
                    ...
                    await self.retrieve_metadata_external_xml_negotiated(target_url_list)
                    await self.retrieve_metadata_external_schemaorg_negotiated(target_url_list)
                    await self.retrieve_metadata_external_rdf_negotiated(target_url_list)
                    await self.retrieve_metadata_external_datacite()
                    if not repeat_mode:
                        await self.retrieve_metadata_external_linked_metadata()
                        await self.retrieve_metadata_external_oai_ore()
    ...
    def sort_link_list(self, linklist):
        # prefer links entries from dict which look like the landing page url, actually have an url
        # this happens by sorting
        preferred_links = []
        other_links = []
    ...
    async def extract_embedded_dublin_core(self, html):
        self.logger.info(
            self.ch.get_metric("metadata_properties") + " : Trying to retrieve Dublin Core metadata from html page"
        )
        ...
        if dc_dict:
            ...
            self.logger.log(
                self.LOG_SUCCESS,
                self.ch.get_metric("metadata_properties") + " : Found DublinCore metadata -: " + str(dc_dict.keys()),
            )
        else:
            self.logger.info(self.ch.get_metric("metadata_properties") + " : DublinCore metadata UNAVAILABLE")

    async def extract_embedded_highwire(self, html):
        self.logger.info(
            self.ch.get_metric("metadata_properties")
        ...
        if hw_dict:
            ...
            self.logger.info(self.ch.get_metric("metadata_properties") + " : Highwire or eprints metadata UNAVAILABLE")

    async def extract_embedded_microdata(self, extruct_metadata):
        self.logger.info(
            self.ch.get_metric("metadata_properties") + " : Trying to retrieve Microdata metadata from html page"
        )
        ...
        if micro_dict:
            ...
            self.logger.log(
                self.LOG_SUCCESS,
                self.ch.get_metric("metadata_properties") + " : Found microdata metadata -: " + str(micro_dict.keys()),
            )

    async def extract_embedded_rdfa(self, html):
        # ================== RDFa
        self.logger.info(
            self.ch.get_metric("metadata_properties") + " : Trying to retrieve RDFa metadata from html page"
        )
        ...
        try:
            ...
            if len(rdfa_dict) > 0:
                ...
                self.logger.log(
                    self.LOG_SUCCESS,
                    self.ch.get_metric("metadata_properties") + " : Found RDFa metadata -: " + str(rdfa_dict.keys()),
                )
        except Exception as e:
